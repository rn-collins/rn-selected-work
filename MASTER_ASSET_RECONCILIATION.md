...
## Represented-site recertification R8 — August 13, 2026

- `sport-nsag` — **P only as one public applied-framework concept; assessment and commercial release blocked**. Its five-question heuristic is unvalidated, accepts incomplete answers, and can produce NaN or an unsupported 100% score. “Full Report” is only a contact flow. Remove unfinished placeholders, unsupported science and F1/LA28 implications, and add athlete consent, data, safeguarding, anti-retaliation, clinical-escalation, labor, FERPA, ADA, Title IX, and SafeSport boundaries before promotion.
- `wnh-guide` — **C, release-blocked NSAG Applied component; zero additional standalone count**. The source identifies it as non-standalone, and it contains no delivered guides, report, checkout, evidence database, method, review, or fulfillment. All four displayed DOI identities fail verification and cannot support the cycle-aware THC/dosing claims; correct the NIH/FDA attribution, inert paid-guide and report controls, and clinical/evidence boundaries. Suppress or noindex the live surface until reconstructed.
- `narch-advisory` — **C, release-blocked NSAG Applied component; zero additional standalone count**. Do not imply architectural licensure through “Architect,” firm, or filing language. Correct unsupported study/percentage claims, the thermal-comfort definition, WELL characterization, rescinded GSA P100 reference, privacyless false-success lead capture, and unvalidated diagnostic. A single Research Lead authorship block may use RN's canonical portrait with an explicit non-architect role boundary.

## Represented-site recertification R9 — August 13, 2026

- `transform-observatory` — **P, retain one independent static monitoring prototype; release-blocked**. Eight routes work, but no crawler, database, scheduler, update log, weekly digest, or alerts are demonstrated. Freeze the apparent false DEA psilocybin Schedule III claim pending primary proof; remove Transform/Steve Rolles commission, authorization, delivery, and insider implications; audit the seed endpoint; repair policy/discovery routes and tab canonicals. Do not place portraits across monitoring tabs; an optional single independent-builder About treatment is sufficient.
- `psych-ops-directory` — **P, retain exactly one standalone directory case; release-blocked**. The UI embeds 177 records while an unused API has 178 with duplicate IDs; taxonomy and review dates conflict. Remove fallback/admin-PIN and query-key security, state-changing GETs, open ingestion/analytics, XSS/CSV/privacy/rate defects, and unsupported high-risk record claims. Add RN once in an Editor + Methodology block rather than repeating a portrait in the hero, cards, or footer.
- `set-for-life` — **P, retain one independent product/course-design case; public prelaunch prototype and release-blocked**. Replace “Live self-serve course.” The 938-message/207-reply arithmetic is 22.07%, not 26%, and the seven-position, connection, quote, and causal claims are unverified. Enrollment only opens contact; no course, guide, calendars, Claude Projects, checkout, entitlement, email delivery, updates, or refund workflow exists. Correct stale launch copy, canonical/metadata hosts, false-success contact, privacy/discovery routes, template residue, and platform/FTC boundaries. A single “Who built this” portrait treatment is appropriate.

## Represented-site recertification R10 — August 13, 2026

- `psychonaut-bookworm` — **P, retain exactly one standalone editorial and learning-system case; release-blocked**. Its advertised 1,399 total double-counts 55 mini-essays already inside the 238 Writing records; the dedicated mini-essay collection has only three records. Most flashcards lack citation fields, source fields have no URLs, and 222 “published” records lack outlet URLs. Correct Fat Nugs authorization/hosting language, legal currentness, rights for quotes/source packs, false-success leads, tracking/discovery routes, and accessibility. Use a product screenshot for case/OG and one Editor/Methodology portrait, not repeated portraits.
- `entheogen-atlas` — **P, retain exactly one controlled multidisciplinary atlas prototype; high-risk release-blocked**. It is an embedded-data SPA, not live retrieval. Correct legal/RFRA/RLUIPA/case-status and trial-status errors, overcausal neuroscience, unvalidated phenomenology scores, unsafe ceremony/dose/interaction content, and Indigenous/religious cultural-governance gaps. Repair interaction/accessibility and privacy/discovery routes. An optional single About portrait is sufficient; expert and community reviewers are the more important identity layer. Replace the broken wrong-domain OG image.
